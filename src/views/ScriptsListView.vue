  ...
  <div class="view-container w-full">
    ...
    <main class="region body p-4 m-2 rounded border border-gray-300 bg-white min-h-[200px] dark:bg-[#333333] dark:border-[#404040]">
      ...
      <ul v-if="scripts.length > 0">
        <li v-for="s in scripts" :key="s.id" class="text-gray-800">{{ s.name }} — {{ s.path }}</li>
      </ul>
...
const { scripts, error, busy, addScriptFile, addScriptFolder } = useScripts({ repository, picker, scanner });
